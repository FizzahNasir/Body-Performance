...
class KNNClassifier:
    def __init__(self, k=3, distance_metric=euclidean_distance):
        ...
        distance_metric (function): Function to compute distance between points.
        """
        self.k = k
    ...
    def get_neighbours(self, test_point):
        """
        Get k nearest neighbors for a given test point.

        Parameters:
        test_point (ndarray): A single data point.
...
accuracy_euclidean = calculate_accuracy(y_pred, y_test)
print(f"\nTest Accuracy (Euclidean): {accuracy_euclidean:.2f}")
